...
test('time to string', () => {
    expect(dateAndTime.getTimeAsString({h: 18})).toBe("6:00pm");
});

test('time to string with end hours', () => {
    expect(dateAndTime.getTimeAsString({h:18}, {h:21})).toBe("6:00-9:00pm");
});

test('time to string with end hours and minutes', () => {
    expect(dateAndTime.getTimeAsString({h:18}, {h:21, m:30})).toBe("6:00-9:30pm");
});
